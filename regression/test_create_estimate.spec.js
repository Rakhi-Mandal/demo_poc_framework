...
test('generated flow', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Estimates/Quotes', exact: true })).toBeEnabled();
  await page.getByRole('link', { name: 'Estimates/Quotes', exact: true }).click();
  await page.waitForLoadState('domcontentloaded');

  await expect(page.getByRole('link', { name: 'New Estimate', exact: true })).toBeEnabled();
